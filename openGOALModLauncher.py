...
def getRefreshedTableData(sort_col_idx):
  ...
  mod_table_data = []
  installed_mod_subfolders = {f.name: f.stat().st_mtime for f in os.scandir(ModFolderPATH) if f.is_dir()}
...
layout = [
  ...
  [
    sg.Table(values=LATEST_TABLE_DATA, headings=table_headings, visible_column_map=col_vis, col_widths=col_width, auto_size_columns=False, num_rows=15, 
              text_color="black", background_color="lightblue", alternating_row_color='white', justification="left", selected_row_colors='black on yellow',
              key='-MODTABLE-', expand_x=True, expand_y=True, enable_click_events=True)
  ]
]
...
def handleModTableSelection(row):
  ...
  window['-LAUNCH-'].update("Install" if mod_install_date == "Not Installed" else "Launch")
  ...
  try:
    ...
    if r == 200:
        ...
        window['-SELECTEDMODIMAGE-'].update(
            githubUtils.resize_image(png_data))
        # prints the int of the status code. Find more at httpstatusrappers.com :)
    else:
        window['-SELECTEDMODIMAGE-'].update(
            githubUtils.resize_image(noimagefile))
...
while True:
  ...
  if isinstance(event, tuple):
    ...
    if tmpModSelected in subfolders:
      ...
      sg.Popup('Selected mod is not installed',
